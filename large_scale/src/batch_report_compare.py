...
def get_component_report_detail(component_report_path):
    libraries = []
    libraryversions = []
    pretty_log(f"start processing {component_report_path}")
    with open(component_report_path, mode="r", encoding="UTF-8") as csv_file:
        ...
        for row in csv_reader:
            ...
            raw_library = row["Library"].replace("None", "").replace(": ", ":").replace("\"", '').replace("\'", '').strip().strip('-')
...
def get_issue_report_detail(issue_report_path):
    cve_list = []
    libraryversion_list = []
    pretty_log(f"start processing {issue_report_path}")
...
def file_is_empty(file_path):
    if os.path.getsize(file_path) < 130:
        return True
    else: 
        return False


def generate_sca_tools_report(working_dir: str, report_folder_name, comparatee_list, test_case=''):
    for comparatee in comparatee_list:
        current_report = {
            "comparator": 'groundtruth',
        ...
        comparator_component_report = os.path.join(working_dir, 'groundtruth', f"groundtruth-component-{test_case}.csv")
        comparator_issue_report = os.path.join(working_dir, 'groundtruth', f"groundtruth-issue-{test_case}.csv")
        comparatee_component_report = os.path.join(working_dir, report_folder_name, f"{comparatee}-component-{test_case}.csv")
        comparatee_issue_report = os.path.join(working_dir, report_folder_name, f"{comparatee}-issue-{test_case}-verified.csv")
        if os.path.isfile(comparatee_component_report) and not file_is_empty(comparatee_component_report) \
            and os.path.isfile(comparator_component_report) and not file_is_empty(comparator_component_report):
        ...
        if os.path.isfile(comparatee_issue_report) and not file_is_empty(comparatee_issue_report)\
            and os.path.isfile(comparator_issue_report) and not file_is_empty(comparator_issue_report):
        ...
        testsuite_report_summary_path = os.path.join(working_dir,report_folder_name, f"groundtruth-{comparatee}-summary.json")
...
def clear_json(working_dir, report_folder_name):
    tm_path = os.path.join(working_dir, report_folder_name)
...
if __name__ == '__main__':
    manifest_csv_path = f"/home/lida/SCAEvaluation-main/large_scale/manifest/total_manifest.csv"
    ...
    for cnt, row in enumerate(csv_reader):
        ...
        scan_type = row['type']
        working_dir = os.path.join(report_path, test_case)
        if scan_type == "build":
            comparatee_list = ['scantist', 'owasp','steady', 'ossindex']
            generate_sca_tools_report(working_dir, scan_type, comparatee_list, test_case)
        elif scan_type == 'prebuild':
            comparatee_list = ['scantist', 'dependabot', 'snyk']
            report_folder_name = "prebuild"
            generate_sca_tools_report(working_dir, scan_type, comparatee_list, test_case)
    manifest_csv.close()