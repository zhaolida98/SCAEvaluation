...
def count_features(pom_path, total_count):
    ...
    if has_type_source:
        total_count['type_source_cnt'] += 1
    if has_type_javadoc:
        total_count['type_javadoc_cnt'] += 1
    if has_type_other:
        total_count['type_other_cnt'] += 1
      

    classifier_list = get_classifier_list(tree)
    has_classifier_source = False
    ...
    if has_classifier_source:
        total_count['classifier_source_cnt'] += 1
    ...
    if has_classifier_other:
        total_count['classifier_other_cnt'] += 1
...
if __name__ == "__main__":
    root_folder = "/root/SCAEvaluation/large_scale/projects"
    load_count = False
    ...
    for i, pom_path in enumerate(pom_path_list):
        print(f"{i+1}/{len(pom_path_list)}: {pom_path}", end="\r", flush=True)
        try:
