...
import re
import json
from query_cve_list import get_issue_list_with_cache
from util.utils import exec_command, pretty_log

shadedjar_cache_path = "/root/SCAEvaluation/large_scale/cache/shadedjar_cache.json"
cve_cache_path = "/root/SCAEvaluation/large_scale/cache/cve_cache.json"
gt_generator_path = "/root/ground-truth-generator.jar"
...
def generate_mvn_dep_tree_gt(mvn_dep_file_path, mvn_build_classpath, output_dir, name):
    ...
    try: 
        if os.path.isfile(shadedjar_cache_path):
            cache_file = open(shadedjar_cache_path,'r')
            shadedjar_cache = json.loads(cache_file.read())
...
def convert_line_to_dep(line: str, level: int):
    ...
    if len(maven_parts) < 4:
        pretty_log(f"maven parts should never below 4, line: {line}")
        return None
...
def add_to_report(total_dep, tmp_dep):
    ...
    if type in ['pom']:
        ...
        total_dep['unrelated'][gav] = tmp_dep
    elif 'SNAPSHOT' in gav:
        total_dep['unrelated'][gav] = tmp_dep
    elif classifier in ['javadoc', 'test']:
        total_dep['unrelated'][gav] = tmp_dep
    elif "parent classifier" in exclude_reason:
        total_dep['unrelated'][gav] = tmp_dep
    elif scope == 'shaded':
